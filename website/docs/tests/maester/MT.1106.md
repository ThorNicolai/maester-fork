---
title: MT.1106 - Users should not have never-used Phone (SMS or Voice) authentication methods.
description: This test checks if users have Phone (SMS or Voice) authentication methods that have never been used.
slug: /tests/MT.1106
...
This test checks if users have Phone (SMS or Voice) authentication methods registered that have never been used.

Phone authentication methods that are registered but have never been used may indicate:
...
The test retrieves authentication methods for each user and evaluates the `lastUsedDateTime` property to identify Phone (SMS or Voice) methods that have never been used.

## How to fix

If this test fails, you should:

1. **Review the identified users and their never-used phone methods** - Determine if the methods are still needed
...
3. Select the user with the never-used phone method
